...
import { AlertTriangle, TrendingUp, TrendingDown, Package, Users, ShoppingBag, Brain, Settings, LayoutDashboard, Bell, ChevronRight, Search, Send, Loader2, ArrowUpRight, ArrowDownRight, Minus, Zap, ChevronLeft } from "lucide-react";

// ─── BRAND TOKENS (matches PPT exactly) ───────────────────────
const T = {
  teal:      "#0891B2",
  tealDark:  "#065F73",
  tealLight: "#E0F7FA",
...
const CITIES  = { "Maharashtra":["Mumbai","Pune","Nagpur","Nashik"], "Delhi NCR":["New Delhi","Gurgaon","Noida","Faridabad"], "Karnataka":["Bangalore","Mysore","Hubli"], "Tamil Nadu":["Chennai","Coimbatore","Madurai"], "Gujarat":["Ahmedabad","Surat","Vadodara"], "Telangana":["Hyderabad","Warangal"], "West Bengal":["Kolkata","Howrah"], "Rajasthan":["Jaipur","Jodhpur","Udaipur"] };
const NAMES   = ["Sharma Distribution Co","Patel Enterprises","Singh & Sons Trading","Reddy Retail Solutions","Gupta FMCG Distributors","Mehta Supply Chain","Iyer & Associates","Khan Wholesale Traders","Joshi Marketing Agency","Nair Consumer Goods","Agarwal Dist. Network","Chatterjee Trading Co","Pillai Retail Hub","Saxena Distribution","Deshmukh Enterprises","Malhotra Trading Co","Choudhary Suppliers","Banerjee & Co","Kulkarni Distribution","Thakur Wholesale","Verma Trading Corp","Mishra Supply Hub","Pandey Dist. Services","Bhat Marketing Pvt","Deshpande Traders","Kapoor Retail Network","Sethi Distribution","Rajput Trading House","Menon Supply Co","Bhatt Enterprises"];

function mkDistributors() {
  return NAMES.map((name, i) => {
    const region = REGIONS[i % REGIONS.length];
    const city   = CITIES[region][i % CITIES[region].length];
    const bad    = i < 4;
    const prim   = Math.floor(8000 + Math.random() * 12000);
    const str    = bad ? 0.35 + Math.random() * 0.25 : 0.75 + Math.random() * 0.2;
    const sec    = Math.floor(prim * str);
    const days   = Math.max(5, Math.floor((prim - sec) / (sec / 30)));
    const risk   = Math.min(100, Math.round(days > 60 ? 85 + Math.random()*15 : days > 40 ? 55 + Math.random()*25 : days > 25 ? 20 + Math.random()*20 : Math.random()*15));
    return { id:`D${String(i+1).padStart(3,"0")}`, name, region, city, primaryUnits:prim, secondaryUnits:sec, sellThrough:+(sec/prim).toFixed(2), days, risk, trend: bad?"declining": Math.random()>0.3?"improving":"stable", topSKUs:SKUS.slice(0,3+Math.floor(Math.random()*4)).map(s=>s.name) };
  });
}
...
// ─── HELPERS ──────────────────────────────────────────────────
function riskColor(score) { return score>=70?T.red:score>=40?T.amber:T.green; }
function riskBg(score)    { return score>=70?T.redLight:score>=40?T.amberLight:T.greenLight; }
function riskLabel(score) { return score>=70?"Critical":score>=40?"At Risk":"Healthy"; }
function sevColor(s)      { return s==="critical"?T.red:s==="warning"?T.amber:T.teal; }
function sevBg(s)         { return s==="critical"?T.redLight:s==="warning"?T.amberLight:T.tealLight; }
function sevLabel(s)      { return s==="critical"?"Critical":s==="warning"?"Warning":"Info"; }
function trendIcon(t)     { if(t==="up"||t==="improving") return <ArrowUpRight size={13} style={{color:T.green}}/>; if(t==="down"||t==="declining") return <ArrowDownRight size={13} style={{color:T.red}}/>; return <Minus size={13} style={{color:T.textDim}}/>; }
...
function NavItem({id,label,icon:Icon,badge,active,onClick}) {
  return (
    <button onClick={()=>onClick(id)} style={{width:"100%",display:"flex",alignItems:"center",gap:8,padding:"8px 12px",borderRadius:8,border:"none",cursor:"pointer",textAlign:"left",fontFamily:"inherit",fontSize:13,fontWeight:active?600:400,color:active?T.teal:T.textLight,background:active?T.tealLight:"transparent",transition:"all 0.15s",marginBottom:2}}>
      <Icon size={15} style={{flexShrink:0,color:active?T.teal:T.textDim}} />
      <span style={{flex:1}}>{label}</span>
      {badge>0 && <span style={{background:T.red,color:"#fff",borderRadius:99,fontSize:9,fontWeight:700,padding:"1px 6px",minWidth:16,textAlign:"center"}}>{badge}</span>}
    </button>
  );
}
...
// ─── VIEWS ────────────────────────────────────────────────────
function OverviewView({onDistClick}){
      ...
      <div style={{display:"flex",flexDirection:"column",gap:18}}>
        ...
        <Card>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {d.topSKUs.map((name,i)=>{const s=SKUS.find(x=>x.name===name);return s?(<div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",background:T.bg,borderRadius:8}}><span style={{fontSize:12,color:T.navy,fontWeight:500}}>{s.name}</span><div style={{display:"flex",alignItems:"center",gap:12,fontSize:11,color:T.textLight}}><span>₹{s.asp}</span><span>Returns: {s.ret}%</span>{trendIcon(s.trend)}</div></div>):null;})}
          </div>
        </Card>
      </div>
    );
    ...
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <Card style={{padding:0,overflow:"hidden"}}>
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 0.8fr 0.8fr 0.9fr 0.7fr 60px",gap:0,padding:"10px 20px",background:T.bg,borderBottom:`1px solid ${T.border}`}}>
          {["Distributor","Region","Sell-Through","Inv. Days","Units","Risk",""].map(h=><div key={h} style={{fontSize:10,fontWeight:600,color:T.textDim,textTransform:"uppercase",letterSpacing:"0.05em"}}>{h}</div>)}
        </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <Card style={{padding:0,overflow:"hidden"}}>
        <div style={{display:"grid",gridTemplateColumns:"2.2fr 1fr 0.8fr 0.9fr 0.9fr 0.8fr 80px",gap:0,padding:"10px 20px",background:T.bg,borderBottom:`1px solid ${T.border}`}}>
          {["Product","Category","MRP","Sell-Through","Return Rate","Dead Stock",""].map(h=><div key={h} style={{fontSize:10,fontWeight:600,color:T.textDim,textTransform:"uppercase",letterSpacing:"0.05em"}}>{h}</div>)}
        </div>
        {sorted.map(s=>{
          ...
          return(
            <div key={s.id} style={{display:"grid",gridTemplateColumns:"2.2fr 1fr 0.8fr 0.9fr 0.9fr 0.8fr 80px",gap:0,padding:"11px 20px",borderBottom:`1px solid ${T.border}`,alignItems:"center"}}>
              ...
              <div style={{fontSize:13,fontWeight:700,color:riskColor(ratio>=0.70?0:ratio>=0.85?0:100),...(ratio>=0.85?{color:T.green}:ratio>=0.70?{color:T.amber}:{color:T.red})}}>{ratio}</div>
...
Respond in 2-3 short paragraphs. Be specific with names and numbers. No bullet points. Use plain prose. Reference the actual data.`;
  ...
  const send=useCallback(async()=>{
    ...
    try {
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:ctx,messages:[...messages.slice(-6).map(m=>({role:m.role,content:m.content})),{role:"user",content:q}]})});
      const d=await r.json();
      const txt=d.content?.filter(b=>b.type==="text").map(b=>b.text).join("\n")||"Unable to generate insight.";
      setMessages(p=>[...p,{role:"assistant",content:txt}]);
    } catch { setMessages(p=>[...p,{role:"assistant",content:"Connect to the Multicommerce backend to activate live AI analysis."}]); }
    setLoading(false);
  },[input,loading,messages]);

  const suggestions=["Which distributors need immediate attention?","What's causing the inventory buildup this quarter?","Which SKUs should we stop promoting offline?","How do we prevent another channel stuffing crisis?"];
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 120px)",gap:16}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        {suggestions.map(q=><button key={q} onClick={()=>setInput(q)} style={{padding:"5px 12px",borderRadius:99,border:`1px solid ${T.border}`,background:T.white,color:T.textLight,fontSize:12,cursor:"pointer",fontFamily:"inherit",transition:"all 0.1s"}}>{q}</button>)}
      </div>
    ...
    <div style={{display:"flex",height:"100vh",width:"100%",background:T.bg,fontFamily:"'DM Sans','Helvetica Neue',system-ui,sans-serif",color:T.text}}>
      ...
      <div style={{width:220,flexShrink:0,background:T.white,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",overflowY:"auto"}}>
        ...
        <div style={{padding:"0 10px 8px"}}>
          ...
          {nav.map(n=><NavItem key={n.id} {...n} active={view===n.id} onClick={id=>{setView(id);setSelDist(null);}}/>)}
        </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:28}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          {view==="dashboard"    && <OverviewView onDistClick={setSelDist}/>}
